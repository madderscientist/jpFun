...
`prepareHorizontal` 每个对象只调用一次；`onPlaced` 会在每轮纵向放置后调用，`head` 等复合协调者还可能在局部平移后再次调用成员的回调，因此它必须基于当前坐标同步，不能累加偏移。`createGeometry` 每次返回独立结果，首轮可被丢弃，第二轮就是最终结果，不再迭代。只有首轮占用参与求解，所以 `createGeometry` 必须是主体坐标的纯函数——相对本轨视觉轴的占用一旦随轮次变化，引擎会静默采用第二轮。

这几个回调不互换：`finalizeLayout` 读取装饰完成后的尺寸（例如 note 的歌词端口），`prepareHorizontal` 读取已准备好的主体，`onPlaced` 才能读取最终位置。`paint` 不重新执行这些步骤，同一布局可以重复绘制且不改变几何。
...
若 `measure` 和 `place` 需要共享测量中间值，策略必须在 lowering 时为本轮创建，状态由本轮对象持有；不能把捕获可变测量结果的闭包保存在 AST 上跨轮复用。`head` 的两侧底边对齐使用这一约定。

`page.ts` 只消费自然行高，产出页面边界和每行全局顶部 `lineTops`，不读取 Track 或具体对象。`@page` 的长度在 parse 时固化为 px，可用内容宽度是页宽减左右边距；`height=0` 表示无限高，有限页面至少容纳一行。完整非末页保持首行贴上边距，把扣除最小行距后的剩余高度均分到每个系统下方；末页保留自然行距。
...
弹簧的原理决定了不挤压或者轻微挤压时和左右边界保持一定距离，而过度挤压时一定会溢出边界，这会导致每行的最左和最右对不齐，特别是末尾为小节线的时候，对不齐的情况格外清楚。解决这个问题不能靠弹簧（无论劲度系数设置多大，永远是近似），因此引入了后处理：忽略最左边和最右边的弹簧，视“可分配间隙”为内容的左实际边界和容器的左边界的差值、内容右实际边界和容器右边界的差值（如果轻微挤压，此值为正；溢出时为负），然后将这段间隙平均分配到每个自由间隙上。