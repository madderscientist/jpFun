---
title: 编辑器集成
description: 应用层的数据依赖、结果有效期、源码映射与设备适配。
sidebar:
  order: 8
---

[playground](https://github.com/madderscientist/jpFun/tree/HEAD/apps/playground/) 是核心库的应用层集成。编辑器负责维护文档状态、协调不同有效期的编译结果，并将几何与播放计划接到浏览器界面和设备；它不定义另一套记谱语义。

本文先说明数据依赖与结果生命周期，再介绍 playground 的具体功能实现。VS Code 部分提供扩展点的接入建议，相关插件尚未实现。
...
| 状态 | 来源与有效期 |
| --- | --- |
| 源码与编辑选区 | 编辑器文档模型，是应用的输入状态 |
| token 与调用边界 | 当前源码的词法结果，不依赖成功排版 |
| AST、Lowering、布局与诊断 | 一次完整编译的结果，不与其他源码版本混用 |
| 源码与谱面映射 | 从同轮源码区间和最终几何建立，源码变化后立即失效 |
| 播放计划 | 从成功编译的 Lowering 按需生成，同一源码版本复用 |
| 缩放、面板、混音与设备状态 | 应用状态，不回写 AST 或改变核心记谱语义 |

结果有效期不仅影响预览更新，也决定能否执行定位、去糖替换和播放。旧谱面上的坐标不能与新源码的偏移混用；异步接入时，同样需要防止较早请求的结果覆盖较新的文档状态。
...
`Ctrl`/`Cmd` + 点击标签引用时，光标跳到被标注对象，便于查看引用对应的音符或其他对象。按住修饰键时，可跳转的标签显示下划线和手型。只有引用支持跳转，声明、音符和注释中的同名文字不响应。
...
因此，`F2` 按绑定到同一个声明的引用分组。跳转和重命名可共用声明查找逻辑：跳转查找引用绑定的声明，重命名收集绑定到该声明的全部引用。
...
`PlaybackPlan.tracks[index]`、混音设置与 tinySynth 的 `channel[index]` 一一对应。这里的 channel 标识浏览器内的混音输出，MIDI 通道编号由导出适配器另行分配。音量、Mute 和 Solo 直接作用于原声部的输出增益，无需重建计划。
...
`X` 与 `9` 等价，谱面均显示 `X`。它们产生真实的 NoteOn/NoteOff：NoteOn 携带 `percussion: true`、`midi: 37` 和原声部身份，NoteOff 通过 `noteId` 配对。`37` 在这里是 GM Side Stick 的打击键号；`0/Z` 和 `8` 仍不产生音符事件。
...
前端为 tinySynth 补充了一个固定频率、快速衰减的合成音色，近似表现 Side Stick 的短促敲击，调用现有的 `play()`，复用调度、混音和 `stopAll()`。从 `X` 的延续区间中途 seek 或恢复播放时，不重新触发已经过去的敲击；回到起音位置则正常触发。
...
多个原声部的 `X` 共用 MIDI channel 10，不能分别向这个共享通道写不同的 CC7。导出器将鼓通道的 CC7 固定为 `127`，再按各原声部音量缩放打击力度；音量为零时整对省略 NoteOn/NoteOff，不产生空鼓轨。`MTrk` 是文件中的轨道块，MIDI channel 是事件中的通道编号，两者独立分配。
...
打击乐目前支持播放和导出。MIDI/MusicXML 导入器仍跳过鼓通道，导出的 `X` 再导入后会丢失。
...
- **全文格式化**和**代码折叠**：已有回车时为上一行补空格的功能，但不提供完整文档的格式化命令。
